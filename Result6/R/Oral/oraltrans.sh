...
path_result=/home/liusai/24/space/result/oral/metatranscript
...
# Bowtie2比对
cat oral_id.txt | while read id; do
...
# Prodigal 基因预测
cat oral_id.txt | while IFS= read -r id || [ -n "$id" ]; do
    echo "Start processing ${id}: $(date +"%Y-%m-%d %T")"
    id=$(echo "$id" | tr -d '\r')

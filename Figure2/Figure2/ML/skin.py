...
df = pd.read_csv(data_folder / "skin_machine.csv")
X = df.iloc[:, 2:]
y = df["Group"]
...
classifiers = {
    "Random Forest": RandomForestClassifier(random_state=42, n_estimators=200, class_weight="balanced", max_depth=5, min_samples_split=5),
    "SVM": SVC(random_state=41, C=9,probability=True, class_weight="balanced"),
    "Gradient Boosting": GradientBoostingClassifier(random_state=42, max_depth=5, subsample=0.8),
    "Logistic Regression": LogisticRegression(max_iter=1500, random_state=55)
}
cross_val_results = {}
holdout_results = {}
holdout_predictions = {}
